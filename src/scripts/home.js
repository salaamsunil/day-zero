// Homepage-only behaviour: typewriter, reveals, counters, forms, chatbot.
// Shared nav/header behaviour lives in nav.js.

// Video placeholder loader (used by product-card video embeds, if present)
window.loadVideo = function loadVideo(containerId, embedUrl) {
...
function initHome() {
        ...
        waitlistForm.addEventListener('submit', async (e) => {
            ...
            try {
                const res = await fetch('https://formsubmit.co/ajax/hello@day-zero.com.au', {
                    ...
                    body: JSON.stringify({ email: email, _subject: 'QueueZero Waitlist Signup', _template: 'table' })
                });
                if (res.ok) {
                    btn.textContent = "You're on the list!";
                    btn.style.background = '#10B981';
                    waitlistForm.reset();
                    setTimeout(() => { btn.textContent = originalText; btn.disabled = false; btn.style.background = ''; }, 4000);
    ...
    if (demoForm) {
        demoForm.addEventListener('submit', async (e) => {
            ...
            const productLabels = { nutricare: 'NutriCare', restroai: 'RestroAI', queuezero: 'QueueZero', multiple: 'Multiple products' };
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    btn.style.background = '#10B981';
                    demoForm.reset();
                    setTimeout(() => { btn.innerHTML = originalHTML; btn.disabled = false; btn.style.background = ''; }, 5000);
                } else { throw new Error('Submission failed'); }
            } catch {
                btn.innerHTML = '<i class="fas fa-exclamation-circle"></i> &nbsp;Something went wrong. Try emailing us.';
                btn.style.background = '#EF4444';
                setTimeout(() => { btn.innerHTML = originalHTML; btn.disabled = false; btn.style.background = ''; }, 4000);
            }
        });
    }
...
function initChatbot() {
    const RESPONSES = {
        nutricare: {
            text: 'NutriCare is our dietary management platform for aged care facilities. It includes real-time allergen and IDDSI texture conflict detection, NutriAssist AI for clinical Q&A, MUST malnutrition screening, weight tracking, incident logs, a Kitchen Display System, nutritional compliance reports, a family portal, and full multi-site RBAC. It is live at nutri-care.app.',
            cta: { label: 'Book a Demo', action: 'contact' }
        },
        restroai: {
            text: 'RestroAI is our restaurant operating system, live at restroai.app. It covers AI insights and predictive ordering, online ordering with QR menus, kitchen display, staff rostering, a clock-in kiosk, HACCP compliance, waste tracking, carbon tracking, and a full inventory and procurement module with 3-way matching.',
            cta: { label: 'Explore RestroAI', action: 'contact' }
        },
        queuezero: {
            text: 'QueueZero is our upcoming mobile ordering and queue management system built for food trucks and pop-ups. Customers order ahead, you cook ahead. Features will include mobile-first ordering, live queue visibility, and order-ahead scheduling.',
    ...
    function showGreeting() {
        botReply(
            'Hi there! I am the Day Zero assistant. How can I help you today?',
            [
    ...
    function startContactFlow() {
        ...
        botReply('I would love to connect you with our team! What is your name?');
    ...
    function handleContactInput(value) {
        ...
        if (contactStep === 'name') {
            ...
            input.type = 'email';
            botReply('Thanks, ' + contactData.name + '! What is your email address?');
            setTimeout(() => input.focus(), 700);
        } else if (contactStep === 'email') {
            if (!value.includes('@')) {
                botReply('That does not look like a valid email. Could you try again?');
                setTimeout(() => input.focus(), 700);
                return;
            }
    ...
    async function submitChatContact() {
        ...
        try {
            ...
            typing.remove();
            botReply(
                'Sorry, I could not send that right now. You can email us directly at hello@day-zero.com.au.',
                [{ label: 'Back to menu', action: 'menu' }]
            );
        }
    }
