...
## Tech Stack
- **Astro 7** (static output, `build: { format: 'file' }`) with `@astrojs/mdx` and `@astrojs/sitemap`. Migrated from plain HTML on 2026-05-30 (studio-hub rebuild); upgraded 6→7 on 2026-08-22. Keep `compressHTML: true` in `astro.config.mjs`: Astro 7's default (`'jsx'`) strips whitespace between inline elements, which breaks the span-heavy hero/nav/footer/pricing copy. MDX now renders via the Sätteri engine (no custom remark/rehype plugins in use).
